...
void preorder(AVLNode *root, int tinggi, BST *set, int max) {
    if (root) {
        if(root->data)
        {
            int jarak = findjrk(set->_root, root->data);
            bst_insert(set, root->data, max - jarak);
        }
        preorder(root->left, tinggi, set, max);
        preorder(root->right, tinggi, set, max);
    }
}
...
int main(){
    ...
    while(t--)
    {
        ...
        else if(cmd == 2)
        {
            scanf("%lld", &cari);
            bst_init(&tree);
            int max = maxDepth(avlku._root); 
            preorder(avlku._root, cari, &tree, max);
