...
void __bst__inorder(BSTNode *root, ll cek) {
    if (root) {
        __bst__inorder(root->left, cek);
        count++;
        if(root->key == cek)
        {
            printf("%lld\n", count);
...
void bst_inorder(BST *bst, ll cek) {
    __bst__inorder(bst->_root, cek);
}
...
int main()
{
    ...
    while(t--)
    {
        ...
        else if(cmd == 2)
        {
            ...
            {
                bst_remove(&set, input);
            }
            else{
                puts("Tidak ada di gudang");
            }
        }
        else{
            if(bst_find(&set, input))
